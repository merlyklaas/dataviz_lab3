...
p_load(here, rio, tidyverse, colorspace, scico, ggplot2, gghighlight)
...
Total Honey Production By State Across Years 1998-2012 (First Attempt - this seems way too busy)
```{r}
...
```{r}
cbt <- ggplot(dat, mapping = aes(x = year, y = prodvalue, color = state)) +
    geom_line()
...
```{r}
cbud <- ggplot(dat, mapping = aes(x = year, y = prodvalue, color = state)) +
    geom_line() +
